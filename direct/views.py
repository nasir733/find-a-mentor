    ...
    if user.user_type == 'Mentor':
        if Room.objects.filter(mentor=user, mentee__username=active_direct).exists():
            room = Room.objects.get(
                mentor=user, mentee__username=active_direct)
        else:
            room_name = user.username+'_'+active_direct
            room = Room.objects.create(
                name=room_name, mentor=user, mentee=User.objects.get(username=active_direct))
        ...
        if Room.objects.filter(mentee=user, mentor__username=active_direct).exists():
            room = Room.objects.get(
                mentee=user, mentor__username=active_direct)
        else:

            room_name = user.username+'_'+active_direct
            room = Room.objects.create(name=room_name, mentee=user, mentor=User.objects.get(
                username=active_direct), menter_username=active_direct, mentee_username=user.username)
            url = "/direct/room/{}".format(room.mentee.username)
            notify.send(user, recipient=room.mentor, verb='Message',
                        description="{} started new conversation".format(user.username), url=url)
    ...
    if request.method == 'POST':
        ...
        if from_user.user_type == "Mentor":
            ...
            message = Message(user=from_user, text=body,
                              room=room, user_name=from_user.username)
            message.save()
            notify.send(from_user, recipient=room.mentee,
                        verb='Message', description=body, url=url)
            ...
            message = Message(user=from_user, text=body,
                              room=room, user_name=from_user.username)
            message.save()
            notify.send(from_user, recipient=room.mentor,
                        verb='Message', description=body, url=url)
        ...
        try:
            messages = Message.objects.filter(
                Q(user=room.mentor) | Q(user=room.mentee), room=room)
