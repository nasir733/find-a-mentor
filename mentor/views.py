...
from meeting.models import MeetingRecording
User = get_user_model()
...
def home(request):
    ...
    mentor_requests = MentorRequest.objects.filter(
...
def profile(request):
    ...
    reviews = Review.objects.filter(
        content__user=request.user.mentorprofile).order_by('?')[:4]
...
def clippedcontent(request):
    context = {}
    recordings = MeetingRecording.objects.filter(mentor=request.user)
    context['recordings'] = recordings
    print(recordings)
    return render(request, 'mentor/clippedcontent.html', context=context)


@login_required(login_url='/mentor/login/')
def mymentees(request):
    my_mentees = MentorMenteeRelations.objects.filter(
        mentor=request.user.mentorprofile)
    context = {}
    ...
    if request.method == 'POST':
        ...
        print(search_text)
        my_mentees = MentorMenteeRelations.objects.filter(Q(mentee__user__username__icontains=search_text) | Q(
            mentee__user__email__icontains=search_text), mentor=request.user.mentorprofile)
        context['my_mentees'] = my_mentees
...
def mentorpage(request, task="view"):
    ...
    if MentorAvailability.objects.filter(mentor=request.user.mentorprofile).exists():
        mentor_working_hours = MentorAvailability.objects.filter(
            mentor=request.user.mentorprofile)
...
def singlecontent(request, id):
    ...
    if content.user == request.user.mentorprofile:
        booked = MentorRequest.objects.filter(
            content=content, accepted=True).count()
        pending = MentorRequest.objects.filter(
            Q(accepted=False) & Q(declined=False), content=content).count()
        declined = MentorRequest.objects.filter(
            declined=True, content=content).count()
        if request.user.user_type == "Mentor":
...
def singlecontentupdate(request, id):
    ...
    context['content'] = content
    if request.user.user_type == "Mentor":
        context['form'] = CreateContentForm(
            instance=request.user.mentorprofile)
        if request.method == 'POST' or request.method == 'FILES':
            title = request.POST.get('title')
            description = request.POST.get('description')
            image = request.FILES.get('image')
...
def menteerequests(request, view="all"):
    ...
    mentor = request.user.mentorprofile
    if view == "all":
        mentee_requests = MentorRequestTime.objects.filter(
            request__mentor=mentor)
        context['mentee_requests'] = mentee_requests
    elif view == "pending":
        mentee_requests = MentorRequestTime.objects.filter(
            request__mentor=mentor, request__accepted=False, request__declined=False)
        context['mentee_requests'] = mentee_requests
    elif view == "accepted":
        mentee_requests = MentorRequestTime.objects.filter(
            request__mentor=mentor, request__accepted=True, request__declined=False)
        context['mentee_requests'] = mentee_requests
    elif view == "declined":
        mentee_requests = MentorRequestTime.objects.filter(
            request__mentor=mentor, request__declined=True)
        context['mentee_requests'] = mentee_requests
...
def addworkinghours(request):
    ...
    if request.method == 'POST':
        ...
        from_hour = datetime.strftime(datetime.strptime(request.POST.get(
            'from_hour') + ":00 " + request.POST.get('from_hour_time'), '%I:%M %p'), "%H:%M")
        to_hour = datetime.strftime(datetime.strptime(request.POST.get(
            'to_hour') + ":00 " + request.POST.get('to_hour_time'), '%I:%M %p'), "%H:%M")
        seq1 = []
        print(from_hour)
        print(to_hour)
        mentorworkinghours = MentorAvailability(
            weekday=weekday, from_hour=from_hour, to_hour=to_hour, mentor=request.user.mentorprofile)
        mentorworkinghours.save()

        diff_hour = datetime.strptime(
            to_hour, "%H:%M") - datetime.strptime(from_hour, "%H:%M")
        ...
        for x in range(breaks):
            ...
            time_slot = MentorRequestTimeSlot(mentor_availability=mentorworkinghours, from_time=single_from_time,
                                              to_time=single_to_time, weekday=weekday, mentor=request.user.mentorprofile)
            time_slot.save()
...
def catergorymentee(request, category):
    context = {}
    mentee_interests = MenteeInterest.objects.filter(
        interest__catergory__name__icontains=category).distinct()
...
def updatemeetingurl(request, meeting_id):
    ...
    if request.method == 'POST':
        ...
        meeting.save()
        message = "{} Session Updated with New Meeting by {}".format(
            meeting.content.title, meeting.mentor.user.username)
...
def completedsessions(request):
    ...
    booked = Meeting.objects.filter(
        mentor=request.user.mentorprofile, completed=True)
    print(booked)
    context['booked'] = booked
