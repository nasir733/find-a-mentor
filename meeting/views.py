...
class upload_meeting_recording(viewsets.ModelViewSet):
...
def connectcallmentee(request, meeting_id):
    ...
    context['meeting_time'] = (
        meeting.mentor_time_slot.from_time).strftime('%H:%M:%S')
    print(time_in_range((meeting.mentor_time_slot.from_time).strftime('%H:%M:%S'),
          (meeting.mentor_time_slot.to_time).strftime('%H:%M:%S'), datetime.datetime.now().strftime('%H:%M:%S')))
    ...
    context['timepassed'] = time_in_range((meeting.mentor_time_slot.from_time).strftime(
        '%H:%M:%S'), (meeting.mentor_time_slot.to_time).strftime('%H:%M:%S'), datetime.datetime.now().strftime('%H:%M:%S'))
    print(meeting.mentee.user.username)
...
def completemeeting(request, meeting_id):
    ...
    meeting.save()
    mentor_booked_event = MentorBookedEvent.objects.get(
        mentor_time_slot=meeting.mentor_time_slot)
    mentor_booked_event.is_available_status = True
    mentor_booked_event.save()
    url = "/dashboard/content/{}".format(meeting.content.id)
    message = "{} request session completed".format(
        meeting.mentor.user.username)
    notify.send(meeting.mentor.user, recipient=meeting.mentee.user,
                verb='Leave a Review for Content', description=message, url=url)
...
def schedulemeeting(request, mentorequest_id):
    ...
    if request.method == "POST":
        ...
        meeting_title = mentee_request.content.title + \
            " requested by " + mentee_request.mentee.user.username
        meeting = Meeting(mentor=mentee_request.mentor, mentee=mentee_request.mentee, meeting_title=meeting_title, meeting_subject=meeting_title,
                          start_time=startime, end_time=endtime, date=mentee_request.mentorrequesttime.date, meeting_request=mentee_request, content=mentee_request.content)
        meeting.save()
        mentee_request.accepted = True
        mentee_request.save()
        url = "/meeting/connectcallmentee/{}".format(meeting.id)
        message = "{} request accepted request for {}".format(
            meeting.mentor.user.username, meeting.content.title)
        notify.send(meeting.mentor.user, recipient=meeting.mentee.user,
                    verb='Call Accepted', description=message, url=url)

        if MentorMenteeRelations.objects.filter(mentor=request.user.mentorprofile, mentee=mentee_request.mentee).exists():
            mentormentee = MentorMenteeRelations.objects.get(
                mentor=request.user.mentorprofile, mentee=mentee_request.mentee)
            print("related")
            mentormentee.save()
        else:
            print("no related")
            relation = MentorMenteeRelations.objects.create(
                mentor=request.user.mentorprofile, mentee=mentee_request.mentee, amount=mentee_request.total_amount)
            relation.save()
...
def create_recording(request):
    ...
    meeting_recording = MeetingRecording(
        sid=sid, resourceId=resourceId, meeting=meeting, mentor=mentor, content=meeting.content)
